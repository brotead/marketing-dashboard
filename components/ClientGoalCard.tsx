...
function GoalRow({ goal, pacing, autoValue, autoSource, onEdit, onDelete, onUpdateOverride }: GoalRowData) {
  ...
  return (
    <div className="flex flex-col gap-3 pt-4 border-t border-gray-200 dark:border-[#2a2a2a]">
      {/* KPI row header */}
      ...
      <div className="grid grid-cols-3 gap-2">
        <div>
          <p className="text-[11px] font-medium text-gray-400 dark:text-gray-500 mb-0.5">Meta mensual</p>
          <p className="font-bold text-gray-900 dark:text-white text-sm tabular-nums tracking-tight">{pacing.goalValue.toLocaleString()}</p>
        </div>
        <div>
          <p className="text-[11px] font-medium text-gray-400 dark:text-gray-500 mb-0.5">Esperado hoy</p>
          <p className="font-bold text-gray-600 dark:text-gray-300 text-sm tabular-nums tracking-tight">{pacing.expectedToDate.toLocaleString()}</p>
        </div>
        <div>
          <p className="text-[11px] font-medium text-gray-400 dark:text-gray-500 mb-0.5">Real hoy</p>
          <div className="flex items-baseline gap-1">
            <p className={`font-bold text-sm tabular-nums tracking-tight ${pacing.diff >= 0 ? 'text-emerald-600 dark:text-emerald-400' : 'text-red-600 dark:text-red-400'}`}>
              {pacing.currentValue.toLocaleString()}
            </p>
            <span className={`text-[10px] tabular-nums ${pacing.diff >= 0 ? 'text-emerald-500' : 'text-red-500'}`}>
      ...
      <div>
        ...
        <div className="relative h-2 bg-gray-200 dark:bg-[#2d2d2d] rounded-full overflow-hidden">
          <div
      ...
      <div className="bg-gray-50 dark:bg-[#252525] rounded-xl p-2.5">
        {!editing ? (
          <div className="flex items-center justify-between gap-2">
            <div>
          ...
          <div className="flex items-center gap-2">
            ...
            <button onClick={handleSave} className="bg-blue-600 text-white px-3 py-1.5 rounded-lg text-xs font-medium hover:bg-blue-700 shrink-0">OK</button>
...
const ClientGoalCard = memo(function ClientGoalCard({ clientName, goals, canEdit, onAddGoal }: Props) {
  ...
  return (
    <div className={`${cfg.bg} bg-white dark:bg-[#1a1a1a] rounded-2xl border ${cfg.border} p-5 flex flex-col shadow-sm`}>
      {/* Client header */}
      <div className="flex items-center justify-between mb-1">
        <p className="font-bold text-gray-900 dark:text-white text-lg tracking-tight">{clientName}</p>
        <span className={`${cfg.badge} text-xs px-2 py-0.5 rounded-full font-medium flex items-center gap-1 shrink-0`}>
          <cfg.Icon size={10} />
          {cfg.label}
        </span>
      </div>
      ...
      {/* Add another goal */}
      {canEdit && (
        <div className="pt-3 mt-2 border-t border-gray-200 dark:border-[#2a2a2a]">
          <button

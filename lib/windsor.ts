...
export async function fetchWindsorAccounts(
  ...
  // Compute "7 days ago" for recent activity check
  ...
  // Only Meta (facebook)
  const meta = records.filter((r) => r.source === 'facebook')
  ...
  for (const r of meta) {
    ...
    if (!map.has(r.account_id)) {
      map.set(r.account_id, {
        ...
        spend: 0,
        recent_spend: 0,
        campaign_count: new Set<string>().size,
      })
    }
  ...
  // Count unique campaigns per account
  const campaignSets = new Map<string, Set<string>>()
  for (const r of meta) {
    if (!r.account_id) continue
    if (!campaignSets.has(r.account_id)) campaignSets.set(r.account_id, new Set())

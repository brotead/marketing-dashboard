...
async function autoSyncCampaigns(
  ...
  for (const entries of wcToEntries.values()) {
    ...
    // Keep entries with real budget or manual override; auto-remove trivial zero duplicates
    const withBudget  = entries.filter(b => b.budget_total > 0 || b.spend_override != null)
    const trivialZero = entries.filter(b => b.budget_total === 0 && b.spend_override == null)
    if (withBudget.length > 0) trivialZero.forEach(b => deleteSet.add(b.campaign_id))
  }
...
export default function CashflowPage() {
  ...
  function getClients(source: Source): string[] {
    ...
    return all.sort((a, b) => {
      const aCampaigns = deduplicateBudgets(monthBudgets.filter(e => e.client_name === a && e.source === source && e.campaign_name !== '__auto__'))
      const bCampaigns = deduplicateBudgets(monthBudgets.filter(e => e.client_name === b && e.source === source && e.campaign_name !== '__auto__'))
  ...
  function clientDotColor(clientName: string, source: Source): string {
    ...
    const deduped = deduplicateBudgets(monthBudgets.filter((b) => b.client_name === clientName && b.source === source && b.campaign_name !== '__auto__'))
    const cb = deduped.filter(b => !b.paused)
    ...
    return (
      <div className="space-y-0.5">
        {clients.map((client) => {
          ...
          const campaigns  = deduplicateBudgets(monthBudgets.filter(e => e.client_name === client && e.source === source && e.campaign_name !== '__auto__'))
